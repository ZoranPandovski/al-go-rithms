...
'''
PROBLEM:
for given list we have to replace the element with the greatest element closest to it's right other wise replace it with -1.
example:
    given : [1,5,3,4]
    for the first element 1(index 0) 5, 3,4 are greater(but we need smaller element) so we replace it with -1 
    likewise for 5(index 1) the closest number smaller than 5 is 3 therefore replace it with 3 and so on ..
...
for i in range(len(given_list)-1,-1,-1): #taking the list is reverse order
    ...
    if comparison_stack.empty()==True:
        ans_list.append(-1)  #empty stack symbolises no element is smaller  so append  -1
    else:
        if comparison_stack.top() < given_list[i]: # check if top is smaller
            ans_list.append(comparison_stack.top()) # top is smaller than append top into answer
        else:
            while ((not comparison_stack.empty()) and comparison_stack.top()>=given_list[i]):
                # pop until we reach an element smaller than given element 
...
print("-----Nearest Smaller Element(NSE)-----")
print("The NSE List: ", ans_list)
